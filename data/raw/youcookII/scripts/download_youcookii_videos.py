...
dataset_root = current_file.resolve().parents[1]
# dataset_root = project_rool / 'data' / 'raw' / 'yoocookII'
vid_file_lst = [dataset_root / 'splits' / 'train_list.txt',
                dataset_root / 'splits' / 'val_list.txt',
                dataset_root / 'splits' / 'test_list.txt']
...
for vid_file, split in zip(vid_file_lst, split_lst):
    ...
    with (open(vid_file) as f):
        ...
        for line in lines:
            ...
            if os.path.exists(vid_prefix + '.mp4') or os.path.exists(vid_prefix + '.mkv') or \
               os.path.exists(vid_prefix + '.webm'):
